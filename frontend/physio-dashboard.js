...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!token || !currentUser) {
        alert('No login session found. Redirecting to physio login...');
        window.location.href = '/physio-auth.html';
        return;
    }
    ...
    if (user.role !== 'physio') {
        alert('You must be logged in as a Physiotherapy Clinic. Current role: ' + user.role);
        window.location.href = '/physio-auth.html';
        return;
    }
...
function displayDoctors() {
    ...
    if (currentDoctors.length === 0) {
        doctorsDiv.innerHTML = '<p>No services added yet. Click "Add Service" to get started.</p>';
        return;
    }

    doctorsDiv.innerHTML = currentDoctors.map(doctor => `
        <div class="doctor-card">
            <img src="${doctor.photo ? doctor.photo : 'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22200%22%3E%3Crect fill=%22%23e0e7ff%22 width=%22200%22 height=%22200%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 font-size=%2224%22 fill=%22%236366f1%22 text-anchor=%22middle%22 dy=%22.3em%22%3EService%3C/text%3E%3C/svg%3E'}" 
                 alt="${doctor.name}" 
                 class="doctor-photo"
                 onerror="this.style.backgroundColor='#e0e7ff'; this.style.color='#6366f1'; this.innerHTML='<div style=&quot;display:flex;align-items:center;justify-content:center;height:100%;font-size:24px;&quot;>Service</div>'"
                 style="display: block; width: 100%; height: auto;">
            <div class="doctor-info">
                <h3>${doctor.name}</h3>
                <p><i class="fas fa-layer-group"></i> <strong>Category:</strong> ${doctor.specialty}</p>
                <p><i class="fas fa-hourglass-half"></i> <strong>Duration:</strong> ${doctor.qualification}</p>
                ${doctor.degrees && doctor.degrees.length > 0 ? `
                    <div class="details-section" style="margin-top: 10px; background: #f8fafc; padding: 10px; border-radius: 8px;">
                        <strong style="font-size: 0.85em; color: #64748b; text-transform: uppercase;">Service Details:</strong>
                        <ul class="degrees-list" style="margin-top: 5px;">
                            ${doctor.degrees.map(d => `<li><strong>${d.degree}:</strong> ${d.institution} ${d.year ? `(${d.year})` : ''}</li>`).join('')}
                        </ul>
                    </div>
                ` : ''}
                <p style="margin-top: 10px;"><i class="fas fa-tools"></i> <strong>Methods:</strong> ${doctor.experience}</p>
                <p><i class="fas fa-rupee-sign"></i> <strong>Service Fee:</strong> ₹${doctor.consultationFee}</p>
                <p><i class="fas fa-calendar-check"></i> <strong>Daily Capacity:</strong> ${doctor.slotsPerDay} slots</p>
                ${doctor.about ? `<p style="border-top: 1px solid #f1f5f9; padding-top: 10px; margin-top: 10px;"><small>${doctor.about}</small></p>` : ''}
            </div>
...
async function loadStats() {
    ...
    const totalDocsElem = document.getElementById('totalDoctors');
    if (totalDocsElem) {
        totalDocsElem.textContent = currentDoctors.length;
    }
...
async function saveDoctor(event) {
    ...
    try {
        ...
        alert(doctorId ? 'Service updated successfully!' : 'Service added successfully!');
        closeModal();
        await loadDoctors();
        await loadStats();
    } catch (error) {
